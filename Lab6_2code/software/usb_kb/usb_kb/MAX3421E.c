...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int returncode;
	returncode = alt_avalon_spi_command(SPI0_BASE, 0, 2, &wdata, 0, NULL, 0);

	//if return code < 0 print an error
	if (returncode < 0)
	{
		printf("Error return code is negative");
	}
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral (see Intel documentation) 
	int returncode;
	returncode = alt_avalon_spi_command(SPI0_BASE, 0, nbytes + 1, &wdata, 0, NULL, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read return code from SPI peripheral (see Intel documentation)
	int returncode;
	returncode = alt_avalon_spi_command(SPI0_BASE, 0, 1, &wdata, 1, &val, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//read return code from SPI peripheral (see Intel documentation)
	int returncode;
	returncode = alt_avalon_spi_command(SPI0_BASE, 0, 1, &wdata, nbytes, data, 0);
